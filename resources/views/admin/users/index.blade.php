    ...
    <div class="p-5 border-b border-slate-100 bg-slate-50/50">
        <form method="GET" class="flex flex-wrap gap-3">
            ...
            <select name="type" class="rounded-xl border-slate-200 shadow-sm focus:border-primary-300 focus:ring focus:ring-primary-200 focus:ring-opacity-50 px-4 py-2.5 text-sm min-w-[160px]">
                ...
                <option value="private_caregiver" {{ request('type') == 'private_caregiver' ? 'selected' : '' }}>Частная сиделка</option>
                <option value="organization" {{ request('type') == 'organization' ? 'selected' : '' }}>Организация</option>
            </select>
        ...
        <table class="w-full">
            <thead>
                <tr class="bg-slate-50 border-b border-slate-100">
                    ...
                    <th class="px-6 py-4 text-left text-xs font-semibold text-slate-500 uppercase tracking-wider">Пользователь</th>
                    <th class="px-6 py-4 text-left text-xs font-semibold text-slate-500 uppercase tracking-wider">Телефон</th>
                ...
                <tr class="table-row hover:bg-slate-50/50">
                    ...
                    <td class="px-6 py-4">
                        <div class="flex items-center gap-3">
                            ...
                            @else
                                <div class="w-10 h-10 rounded-full bg-gradient-to-br from-primary-200 to-primary-400 flex items-center justify-center text-white font-medium text-sm">
                    ...
                    <td class="px-6 py-4">
                        ...
                        @endphp
                        <span class="inline-flex px-2.5 py-1 rounded-lg text-xs font-medium {{ $typeColors[$type] ?? 'bg-slate-100 text-slate-600' }}">
