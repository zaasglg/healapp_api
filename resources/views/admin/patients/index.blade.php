...
@section('title', 'Подопечные')

@section('content')
<div class="flex justify-between items-center mb-8">
    <div>
        <h1 class="text-2xl font-bold text-slate-800">Подопечные</h1>
        <p class="text-slate-500 mt-1">Управление подопечными</p>
    </div>
</div>
...
<div class="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
    ...
    <div class="p-5 border-b border-slate-100 bg-slate-50/50">
        <div class="mb-4 inline-flex rounded-xl border border-slate-200 bg-white p-1">
            <a href="{{ route('admin.patients.index', array_merge($baseTabQuery, ['tab' => 'normal'])) }}"
            ...
            <a href="{{ route('admin.patients.index', array_merge($baseTabQuery, ['tab' => 'test'])) }}"
        ...
        <form method="GET" class="flex flex-wrap gap-3">
            ...
            <div class="flex-1 min-w-[200px]">
                <input type="text" name="search" value="{{ request('search') }}" placeholder="Поиск по имени..."
        ...
        <table class="w-full">
            <thead>
                <tr class="bg-slate-50 border-b border-slate-100">
                    ...
                    <th class="px-6 py-4 text-left text-xs font-semibold text-slate-500 uppercase">Владелец</th>
                    <th class="px-6 py-4 text-left text-xs font-semibold text-slate-500 uppercase">Организация</th>
            ...
            <tbody class="divide-y divide-slate-100">
                ...
                <tr class="table-row hover:bg-slate-50/50">
                    <td class="px-6 py-4 text-sm text-slate-500">#{{ $patient->id }}</td>
                    <td class="px-6 py-4">
                        <div class="flex items-center gap-3">
                            <div class="w-10 h-10 rounded-full bg-gradient-to-br from-rose-200 to-rose-400 flex items-center justify-center text-white font-medium text-sm">
                                {{ mb_substr($patient->first_name, 0, 1) }}{{ mb_substr($patient->last_name, 0, 1) }}
                            </div>
                            <div class="font-medium text-slate-800">{{ $patient->first_name }} {{ $patient->last_name }}</div>
                        </div>
                    </td>
                    ...
                    <td class="px-6 py-4">
                        <div class="flex items-center justify-end gap-2">
                            <a href="{{ route('admin.patients.show', ['patient' => $patient, 'token' => session('admin_token')]) }}" class="p-2 text-slate-400 hover:text-primary-500 hover:bg-primary-50 rounded-lg transition">
                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/></svg>
                            </a>
                            <form method="POST" action="{{ route('admin.patients.destroy', ['patient' => $patient, 'token' => session('admin_token')]) }}" class="inline" onsubmit="return confirm('Удалить?')">
                                @csrf @method('DELETE')
                ...
                @empty
                <tr><td colspan="6" class="px-6 py-12 text-center text-slate-400">Подопечные не найдены</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
    @if($patients->hasPages())<div class="px-6 py-4 border-t border-slate-100 bg-slate-50/50">{{ $patients->links() }}</div>@endif
</div>
@endsection
